...
public sealed class MovimientoRepository(AppDbContext context) : IMovimientoRepository
{
    ...
    public async Task<Movimiento> UpdateMovimientoAsync(Movimiento movimiento, CancellationToken cancellationToken)
    {
    ...
    {
        var movimientoEliminado = movimiento with
        {
            FechaEliminacion = DateTime.UtcNow
        };

        context.Movimientos.Update(movimientoEliminado);
        await context.SaveChangesAsync(cancellationToken);
